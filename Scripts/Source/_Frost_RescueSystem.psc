...
function RegisterForEvents()
	RegisterForModEvent("Frost_OnRescue", "OnRescue")
endFunction
...
bool function RescuePlayer(bool bInWater = false)
	...
	if GetTrackedFollowerCount() > 0
		...
		;Check trees
		if Game.FindClosestReferenceOfAnyTypeInListFromRef(_Camp_Trees, PlayerRef, 3000.0) != none || Game.FindClosestReferenceOfAnyTypeInListFromRef(_DE_DeadwoodList, PlayerRef, 3000.0) != none
...
function GiveRandomSettlementNote(bool Robbed = false)
	if Robbed == false
		PlayerRef.AddItem(_DE_RescueLetterSettlement)
	else
		if rescueDestinationIndex == 400 																;VILLAGE Riverwood
			PlayerRef.AddItem(_DE_RobLetterSettlement_Riverwood)
		elseif rescueDestinationIndex == 402															;VILLAGE Rorikstead
			PlayerRef.AddItem(_DE_RobLetterSettlement_Rorikstead)
		elseif rescueDestinationIndex == 403															;VILLAGE Kynesgrove
			PlayerRef.AddItem(_DE_RobLetterSettlement_Kynesgrove)
		elseif rescueDestinationIndex == 404															;VILLAGE Dragon Bridge
			PlayerRef.AddItem(_DE_RobLetterSettlement_DragonBridge)
		elseif rescueDestinationIndex == 405															;VILLAGE Ivarstead
			PlayerRef.AddItem(_DE_RobLetterSettlement_Ivarstead)
		elseif rescueDestinationIndex == 406															;VILLAGE Nightgate Inn
			PlayerRef.AddItem(_DE_RobLetterSettlement_NightgateInn)
		elseif rescueDestinationIndex == 500															;CITY Winterhold
			PlayerRef.AddItem(_DE_RobLetterSettlement_Winterhold)
		elseif rescueDestinationIndex == 501															;CITY Windhelm
			PlayerRef.AddItem(_DE_RobLetterSettlement_Windhelm)
		elseif rescueDestinationIndex == 502															;CITY Solitude
			PlayerRef.AddItem(_DE_RobLetterSettlement_Solitude)
		elseif rescueDestinationIndex == 503															;CITY Markarth
			PlayerRef.AddItem(_DE_RobLetterSettlement_Markarth)
		elseif rescueDestinationIndex == 504															;CITY Whiterun
			PlayerRef.AddItem(_DE_RobLetterSettlement_Whiterun)
		elseif rescueDestinationIndex == 505															;CITY Riften
			PlayerRef.AddItem(_DE_RobLetterSettlement_Riften)
		elseif rescueDestinationIndex == 506															;CITY Falkreath
			PlayerRef.AddItem(_DE_RobLetterSettlement_Falkreath)
		elseif rescueDestinationIndex == 507															;CITY Morthal
			PlayerRef.AddItem(_DE_RobLetterSettlement_Morthal)
		elseif rescueDestinationIndex == 508															;CITY Dawnstar
			PlayerRef.AddItem(_DE_RobLetterSettlement_Dawnstar)
		endif
	endif
endFunction
...
function RescueAloneTakenHomeToSpouse()		
	GoToSpouse()
	PlayerRef.AddItem(_DE_RescueLetterSpouse)
endFunction
function RescueAloneTakenToOtherAndRobbed()	
	FindNearestOther()
	GoToNearestOther()
	RobPlayer()
	PlayerRef.AddItem(_DE_RobLetterOther)
endFunction
...
function RescueFollowerBuildsCampfire()
	PlayerRef.PlaceAtMe(_DE_Campfire_Simple_Rescue)
